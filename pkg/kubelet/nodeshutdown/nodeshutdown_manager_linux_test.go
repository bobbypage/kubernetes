...
func TestManager(t *testing.T) {
	...
	var tests = []struct {
		...
		{
			desc:                             "basic no override (total=30s, critical=10s)",
		...
		{
			desc:                             "basic no override (total=30, critical=0)",
			activePods:                       []*v1.Pod{normalPod, criticalPod},
			shutdownGracePeriodRequested:     time.Duration(30 * time.Second),
		...
		{
			desc:                             "no override, all time to normal pods",
			activePods:                       []*v1.Pod{normalPod, criticalPod},
		...
		{
			desc:                             "ovverride unsuccesful, zero time",
			activePods:                       []*v1.Pod{normalPod, criticalPod},
			shutdownGracePeriodRequested:     time.Duration(5 * time.Second),
			shutdownGracePeriodCriticalPods:  time.Duration(5 * time.Second),
			...
			expectedDidOverrideInhibitDelay:  true,
			expectedPodToGracePeriodOverride: map[string]int64{"normal-pod": 0, "critical-pod": 0},
		},
	}
